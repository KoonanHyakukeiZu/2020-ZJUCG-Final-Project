...
namespace KooNan
{
	...
	private:
		...
		Mesh terrain_mesh;
	public:
		Terrain(int grid_index_x, int grid_index_z, vector<Texture> textures, float chunk_size = 32.0f, int vertex_count = 32, float if_flatten = false) :
			size(chunk_size), vertex_count(vertex_count), index_x(grid_index_x), index_z(grid_index_z), flatten(if_flatten),
			world_x(index_x * size), world_z(index_z * size), terrain_mesh(generateTerrain(textures)) { }
		void Draw(Shader &shader)
		{
			this->terrain_mesh.Draw(shader);
		}
	private:
		Mesh generateTerrain(vector<Texture> inp_textures, bool flatten = false)
		{
			...
			if (!flatten)
			{
				for (int i = 0; i < vertex_count; i++)
					...
					{
						tmp_vertex->Position = glm::vec3(world_x + ((float)j - vertex_count / 2 + 0.5) / ((float)vertex_count - 1) * size,
							hei_gen.heightsGeneration(j + index_x * (vertex_count - 1), i + index_z * (vertex_count - 1)),
							world_z + ((float)i - vertex_count / 2 + 0.5) / ((float)vertex_count - 1) * size);
